...
public class Util {
	private final static String REL_RULES_PATH = "rulefiles";
	...
	public static List<CartItem> cartItemPresToCartItemList(List<CartItemPres> list) {
		if (list == null)
			return null;
		return list.stream().map(pres -> pres.getCartItem())
	...
	public static List<CatalogPres> catalogListToCatalogPresList(List<Catalog> list) {
		if (list == null)
			return null;
		return list.stream().map(catalog -> {
			CatalogPres catalogPres = new CatalogPres();
			catalogPres.setCatalog(catalog);
			return catalogPres;
		}).collect(Collectors.toList());
	...
	public static List<ProductPres> productListToProductPresList(List<Product> list) {
		...
		return list.stream().map(catalog -> {
			...
			productPres.setProduct(catalog);
			return productPres;
		}).collect(Collectors.toList());
