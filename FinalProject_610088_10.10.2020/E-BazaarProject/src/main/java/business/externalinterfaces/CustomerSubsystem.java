...
public interface CustomerSubsystem {
	...
	/**
	 * 
	 * /** Runs address rules and returns the cleansed address. If a RuleException
	 * is thrown, this represents a validation error and the error message should be
	 * extracted from the exception and displayed
	 */
	 ...
	 * read from the database). Used by other subsystems to read current user's
	 * order history (not used during login process)
	 */
	...
	/**
	 * Called when user submits final order -- customer sends its shopping cart to
	 * order subsystem and order subsystem extracts items from shopping cart and
	 * prepares order
	 */
	 ...
	 * CustomerSubsystemFacade will be out of date; this method should cause order
	 * data to be reloaded
	 */
