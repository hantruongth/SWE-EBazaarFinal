...
public class RulesSubsystemFacade implements RulesSubsystem {
	private final String dirPrefix = (new RulesConfigProperties()).getProperty(RulesConfigKey.DIR_PREFIX.getVal());
	...
	public void runRules(Rules rulesIface) throws BusinessException, RuleException {
		...
		try {
			URL url = this.getClass().getClassLoader().getResource(dirPrefix + nameOfRulesFile);
	...
	/*
	 * this is used if the rules are accessible from this project are not not
	 * encrypted -- not used in EBazaar
	 */
