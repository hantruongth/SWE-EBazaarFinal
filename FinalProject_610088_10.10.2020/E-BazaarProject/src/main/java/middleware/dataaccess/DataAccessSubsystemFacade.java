...
public class DataAccessSubsystemFacade implements DataAccessSubsystem, DataAccessTest {
	...
	/**
	 * Gets a connection from the SimpleConnectionPool and caches it. Also caches
	 * the DbAction instance that is used. Note that all methods in DataAccess
	 * require createConnection as the first step. In saveInTransaction,
	 * createConnection is called explicitly. The save and delete methods have as a
	...
	///// Raw read and save methods -- typically used as part of a bigger
	///// transaction //////

	/**
	 * Precondition: A Connection has been obtained via createConnection. User must
	 * manually releaseConnection after read has completed. Can use atomicRead to
	 * handle createConnection and releaseConnection if no other data access code is
	 * bundled with the read.
	 */
	...
	/**
	 * Precondition: A Connection has been obtained via createConnection. User of
	 * this code must manually releaseConnection after operation has completed. For
	 * updates, returns number of rows affected; for inserts, returns generated key,
	 * if there is one.
	 */
	...
	/**
	 * Precondition: A Connection has been obtained via createConnection. User of
	 * this code must manually releaseConnection after operation has completed.
	 ...
	 * transaction. To wrap multiple or complex sql operations in a transaction, use
	 * startTransaction instead. Note that createConnection is called as part of the
	 * method body (so a separate call to createConnection is not required in this
	...
	/**
	 * This convenience method carries out a typical delete within a transaction. To
	 * wrap multiple or complex sql operations in a transaction, use
	 * startTransaction instead. Note that createConnection is called as part of the
	 * method body (so a separate call to createConnection is not required in this
	 * case). Likewise, releaseConnection is handled automatically.
	 */
	...
	/**
	 * This convenience method performs a single read operation. It handles the
	 * createConnection and releaseConnection steps.
	 */
