...
public class GuiUtils {
	...
	public static double computeTotalInTable(TableView table) {
		...
		DoubleSummaryStatistics summary = items.stream()
				.collect(Collectors.summarizingDouble(item -> Double.parseDouble(item.totalPriceProperty().get())));
	...
	public static ObservableList<OrderItemPres> orderItemsToOrderItemsPres(List<OrderItem> list) {
		return FXCollections.observableList(list.stream().map(orderItem -> {
			OrderItemPres p = new OrderItemPres();
			p.setOrderItem(orderItem);
			return p;
