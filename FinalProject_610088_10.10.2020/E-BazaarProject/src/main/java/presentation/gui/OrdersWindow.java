...
public class OrdersWindow extends Stage implements MessageableWindow {
	...
	public OrdersWindow(Stage primaryStage) {
		...
		// set up table
		TableColumn<OrderPres, String> orderIDCol = TableUtil.makeTableColumn(new OrderPres(), "Order ID",
				"orderIdProperty", 80);
		TableColumn<OrderPres, String> dateCol = TableUtil.makeTableColumn(new OrderPres(), "Date of Order",
				"dateProperty", 100);
		TableColumn<OrderPres, String> totalCostCol = TableUtil.makeTableColumn(new OrderPres(), "Total Price",
