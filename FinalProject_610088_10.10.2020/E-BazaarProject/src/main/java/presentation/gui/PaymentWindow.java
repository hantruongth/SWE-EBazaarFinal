...
public class PaymentWindow extends Stage implements MessageableWindow {
	...
	public CreditCard getCreditCardFromWindow() {
		return CheckoutData.INSTANCE.createCreditCard(nameOnCardField.getText(), expirationField.getText(),
				cardNumberField.getText(), cardTypeField.getValue());
	}

	public PaymentWindow() {
		this.nameOnCardField.setText(CheckoutUIControl.INSTANCE.getShippingBillingWindow().getBillName());
		setTitle("Payment");
		...
		// prepare grid
		List<? extends Node> displayValues = Arrays.asList(nameOnCardField, cardNumberField, cardTypeField,
				expirationField);
		FourByTwoGridPane dataTable = new FourByTwoGridPane(CheckoutData.INSTANCE.getDisplayCredCardFields(),
	...
	private void setCreditCardTypes() {
		ObservableList<String> comboVals = FXCollections.observableList(CheckoutData.INSTANCE.getCredCardTypes());
