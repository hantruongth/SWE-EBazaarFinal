...
public class SelectAddress extends Stage implements MessageableWindow {
	...
	public SelectAddress(ShippingBillingWindow w, CheckoutData.ShipBill shipBill,
		...
		// create columns
		TableColumn<CustomerPres, String> nameCol = TableUtil.makeTableColumn(new CustomerPres(), "Name",
				"fullNameProperty", 100);

		TableColumn<CustomerPres, String> streetCol = TableUtil.makeEditableTableColumn(table, new CustomerPres(),
				"Street", "streetProperty", 100);
		...
		TableColumn<CustomerPres, String> cityCol = TableUtil.makeEditableTableColumn(table, new CustomerPres(), "City",
				"cityProperty", 100);
		...
		TableColumn<CustomerPres, String> stateCol = TableUtil.makeEditableTableColumn(table, new CustomerPres(),
				"State", "stateProperty", 100);
		...
		TableColumn<CustomerPres, String> zipCol = TableUtil.makeEditableTableColumn(table, new CustomerPres(),
				"Zipcode", "zipProperty", 100);
		...
		/*
		 * ObservableList<CartItemPres> tableItems =
		 * BrowseSelectData.INSTANCE.getCartData(); ObservableList<CartItemPres>
		 * selectedItems = table.getSelectionModel().getSelectedItems();
		 * ObservableList<Integer> selectedIndices =
		 * table.getSelectionModel().getSelectedIndices();
		 * 
		 * if(tableItems.isEmpty()) { displayError("Nothing to delete!"); } else if
		 * (selectedIndices == null || selectedIndices.isEmpty()) {
		 * displayError("Please select a row."); } else { boolean result =
		 * BrowseSelectData.INSTANCE.removeFromCart(selectedItems); if(result) {
		 ...
		 * clearMessages(); } else { displayInfo("Zero items removed from cart."); } }
		 */

		deleteButton.setOnAction(evt -> {
			TableUtil.selectByRow(table);
			ObservableList<CustomerPres> tableItems = isShipping ? CheckoutData.INSTANCE.getCustomerShipAddresses()
