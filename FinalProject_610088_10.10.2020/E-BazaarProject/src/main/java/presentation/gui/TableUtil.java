...
public class TableUtil {
	...
	public static <T> TableColumn<T, String> makeTableColumn(T underlyingClass, String colHeader, String nameOfProperty,
			int minWidth) {
		TableColumn<T, String> tableColumn = new TableColumn<>(colHeader);
	...
	public static <T> TableColumn<T, String> makeEditableTableColumn(TableView<T> table, T underlyingClass,
		...
		/*
		 * tableColumn.setOnEditCommit(t -> { T instance =
		 * t.getTableView().getItems().get(t.getTablePosition().getRow()); Class<T> cl =
		 * (Class<T>)underlyingClass.getClass();
		 * 
		 * setValue(field, cl, t.getNewValue(), instance);
		 * System.out.println("value set"); //TableUtil.selectByRow(table); });
		 */
		return tableColumn;
	}

	/*
	 * public static <T> TableColumn<T, String> makeEditableTableColumn(TableView<T>
	 * table, T underlyingClass, String colHeader, String field, int minWidth) {
	 * Callback<TableColumn<T, String>, TableCell<T, String>> cellFactory = new
	 * Callback<TableColumn<T,String>, TableCell<T,String>>() {
	 * 
	 * @Override public TableCell<T, String> call(TableColumn<T, String> p) { return
	 * new EditingCell(field, underlyingClass, table); } }; TableColumn<T, String>
	 * tableColumn = new TableColumn<>(colHeader);
	 * tableColumn.setMinWidth(minWidth); tableColumn.setCellValueFactory( new
	 * PropertyValueFactory<T, String>(field));
	 * tableColumn.setCellFactory(cellFactory); tableColumn.setEditable(true); /*
	 * tableColumn.setOnEditCommit(t -> { T instance =
	 * t.getTableView().getItems().get(t.getTablePosition().getRow()); Class<T> cl =
	 * (Class<T>)underlyingClass.getClass();
	 * 
	...
	public static <T> TableColumn<T, String> makeEditableTableColumnSideEffect(TableView<T> table, T underlyingClass,
			String colHeader, String field, int minWidth, final BiFunction<String, String, String> f, String field2,
			String field3) {
		Callback<TableColumn<T, String>, TableCell<T, String>> cellFactory = new Callback<TableColumn<T, String>, TableCell<T, String>>() {
			@Override
			public TableCell<T, String> call(TableColumn<T, String> p) {
		...
		TableColumn<T, String> tableColumn = new TableColumn<>(colHeader);
		tableColumn.setMinWidth(minWidth);
		tableColumn.setCellValueFactory(new PropertyValueFactory<T, String>(field));
		tableColumn.setCellFactory(cellFactory);
		tableColumn.setEditable(true);
	...
	public static <T> void refreshTable(TableView<T> table, Synchronizer synch) {
		...
		ObservableList<T> newItems = FXCollections.observableList(copy);
	...
	public static <T> TableView.TableViewSelectionModel<T> selectByCell(TableView<T> table) {
		TableView.TableViewSelectionModel<T> selModel = table.getSelectionModel();
